...
var defaultCommandRegistry = mustCommandRegistry([]commandSpec{
	...
	{
		name: "agent", aliases: []string{"agent-query"}, arguments: "[--verbose] [--session ID] [--cwd REMOTE_DIR] [--env KEY=VALUE]... PROMPT",
		listSuffix: "(Ctrl-C cancels the turn)",
		details:    "Send one prompt to the code agent; without --session the last session is resumed; --env overrides the agent child's environment for this turn; --verbose prints tool inputs, outputs, and diffs; the printed query id replays with 'agent-observe'",

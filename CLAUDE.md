...
- `internal/agent` — ACP agent bridge (enabled by default, lazy): one shared `claude-agent-acp` child over the registry's raw-pipe entry; `AgentService.Query` streams turn events; crash → sessions marked lost, next query restarts. Turns carry a query id + per-frame sequence and are persisted frame-by-frame under `--runtime-dir/agent-events/<query-id>/` (segmented log + `state.json`, retained like process logs); the Query stream is an observation window — a client that stops receiving detaches and the turn runs to completion — `CancelQuery` is the explicit stop, and `ObserveQuery` replays/resumes from any sequence (settled, lost-after-restart, or live with follow). `ListQueries` pages retained metadata; `ListSessions` returns only reusable sessions in the current agent generation.
...
- Never log tokens, prompts, or uploaded file contents in *diagnostic* logs (controllerlog/slog); persisted environment stores keys only, never values. Log the *existence* of credentials, never their values. Agent output is different: the query event store persists turn frames (agent messages included) as *data* under the runtime directory — same trust level as process stdout/stderr logs, never mirrored into diagnostic logs.
